...
A curated catalog of contributed valve/handle designs for STEVE. Handle metadata is stored in [handles.json](handles.json) for easy management and extensibility.
...
Create a folder structure under `docs/CAD/Handles/`:
...
- **printedParts** (array of objects):
  - **name**: Part name
  - **path**: Relative path from `docs/CAD/Handles/` (or `null` if TBD)
